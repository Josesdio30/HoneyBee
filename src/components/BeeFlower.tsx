...
export default function BeeFlower() {
  ...
  return (
    <div className="container">
      <div className="grid grid-cols-2" style={{ gap: '4rem', marginBottom: '6rem' }}>
        <motion.div
          ...
          <div className="grid grid-cols-1" style={{ gap: '1rem' }}>
            {flowerList.map((f, idx) => (
              <motion.div 
                ...
                whileHover={{ x: 10 }}
              >
                <div style={{ fontSize: '2rem', background: 'var(--background)', padding: '0.75rem', borderRadius: '16px' }}>🌸</div>
                <div>
                  <h4 style={{ margin: '0' }}>{f.name}</h4>
                  <p style={{ fontSize: '0.85rem', opacity: 0.7 }}>{f.traits}</p>
                  <div style={{ marginTop: '0.5rem', fontSize: '0.75rem', fontWeight: 600, color: 'var(--secondary)', display: 'flex', gap: '0.25rem', alignItems: 'center' }}>
